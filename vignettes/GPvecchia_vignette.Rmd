...
laplace.cov = covfun(locs)
if(m==0)laplace.cov = diag(diag(laplace.cov))
post_lap = calculate_posterior_laplace(z, data.model,C = laplace.cov,return_all = TRUE)
...
if (spatial.dim==1){
  ...
  # ordering unnecessary; we are using a scatter plot rather than lines
  quilt.plot(locs, array(post_lap$mean), main= "True Latent", 
